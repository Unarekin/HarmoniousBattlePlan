...
class InitAllCommand extends Command {
  ...
  trigger(message) {
    return new RegExp("^" + process.env.BOT_COMMAND_PREFIX + "init all$", 'i');
  }
  ...
  exec(message) {
    ...
    return session.run("MATCH (g:Guild {id: {guild}})<-[:Within]-(c:Character) RETURN c", {guild: message.guild.id})
      .then((res) => {
