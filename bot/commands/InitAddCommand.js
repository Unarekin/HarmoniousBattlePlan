...
class InitAddCommand extends Command {
  constructor() {
    super('initAdd', {
    });
  }
  ...
  trigger(message) {
    return new RegExp("^" + process.env.BOT_COMMAND_PREFIX + "init add (.*?)$", 'i')
  }
    ...
    return session.run('MATCH (u:User)-[o:Owns]->(c:Character {id: {character}})-[r:Within]->(g:Guild {id:{guild}}) RETURN u,c', props)
      ...
      .then((res) => {
        ...
        if (res && res.records) {
          ...
          return message.util.send("Increased " + (character.name || character.id) + "'s initiative by " + amount);
