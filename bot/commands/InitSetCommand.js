...
class InitSetCommand extends Command {
  constructor() {
    super('initSet', {
    });
  }
  ...
  trigger(message) {
    return new RegExp("^" + process.env.BOT_COMMAND_PREFIX + "init set (.*?)$", 'i')
  }
    ...
    return session.run('MATCH (u:User)-[o:Owns]->(c:Character {id: {character}})-[r:Within]->(g:Guild {id:{guild}}) RETURN u,c', props)
      .then((res) => {
        if (!res.records[0]) {
          ...
          if (character.owner == user.id || message.member.hasPermission('ADMINISTRATOR')) {
            // Allow
            return session.run("MATCH (c:Character {id: {id}}) SET c.initiative={init} RETURN c", {id: character.id, init: amount});
          } else {
      ...
      .then((res) => {
        ...
        if (res && res.records) {
          ...
          return message.util.send("Set " + (character.name || character.id) + "'s initiative to " + amount);
